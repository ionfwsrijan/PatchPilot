...
from .ml.llm_patcher import generate_patch, OllamaUnavailableError
...
@app.post("/fix-llm", response_model=FixResponse)
async def fix_llm(req: FixRequest):
    job_dir = WORK_ROOT / req.job_id
    repo_dir = job_dir / "repo"
    if not repo_dir.exists():
        raise HTTPException(status_code=404, detail="Unknown job_id")
    ...
    try:
        if not req.finding_ids:
            return FixResponse(job_id=req.job_id, fixes=[])
        
        placeholders = ",".join(["?"] * len(req.finding_ids))
        cursor = await db.execute(
            f"SELECT id, rule_id, severity, message, file_path, line_number FROM findings WHERE job_id = ? AND id IN ({placeholders})",
            (req.job_id, *req.finding_ids,)
        )
        rows = await cursor.fetchall()
        
        for row in rows:
            f_id, rule_id, severity, message, file_path, line_number = row
            ...
            try:
                ...
                prompt = PromptBuilder.build_patch_prompt(finding_meta, file_path, source_code)
                ...
                logger.error(f"Failed to generate LLM patch for finding {f_id}: {e}")
